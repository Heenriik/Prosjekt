...
class HADARMultipleScenes():
    ...
    def __init__(self, root='/home/min/a/sureshbs/Desktop/TeXNet/Dataset/',
        ...
        root = os.path.join(root, "HADAR database") # Uncommented -H
        ...
        for subfolder in SUBFOLDERS:
            ...
            for id in ids_:
                ######### Synthetic #########
                ### Scene11 has different naming: heatcube_0001.mat instead of L_0001_heatcube.mat -H ###
                if subfolder == "Scene11":
                    # Extract frame number from id (L_0001 -> 0001)
                    frame_num = id.split('_')[1]
                    heatcube_filename = f"heatcube_{frame_num}.mat"
                    tmap_filename = f"tMap_{frame_num}.mat"
                    vmap_filename = f"vMap_{frame_num}.mat"
                else:
                    heatcube_filename = f"{id}_heatcube.mat"
                    tmap_filename = f"tMap_{id}.mat"
                    vmap_filename = f"vMap_{id}.mat"
                
                self.S_files.append(os.path.join(root, subfolder, 'HeatCubes', heatcube_filename))
                self.S_beta_files.append(os.path.join(root, subfolder, 'HeatCubes', f'S_EnvObj_{id}.npy'))
                self.T_files.append(os.path.join(root, subfolder, 'GroundTruth', 'tMap', tmap_filename))
                self.e_files.append(os.path.join(root, subfolder, 'GroundTruth', 'eMap', f"new_eMap_{id}.npy"))
                self.v_files.append(os.path.join(root, subfolder, 'GroundTruth', 'vMap', vmap_filename))
